...
var users []model.User // Tambahkan model.
var nextID = 1
...
func cocokPencarian(u model.User, kata string) bool { // Tambahkan model.
...
func listUsers(c *fiber.Ctx) error {
	...
	hasil := []model.User{} // Tambahkan model.
	for _, u := range users {
	...
	return okList(c, "daftar user berhasil diambil", hasil[mulai:akhir], &model.Meta{ // Tambahkan model.
...
func createUser(c *fiber.Ctx) error {
	var req model.CreateUserRequest // Tambahkan model.
	...
	baru := model.User{ // Tambahkan model.
		ID:        nextID,
...
func replaceUser(c *fiber.Ctx) error {
	...
	var req model.ReplaceUserRequest // Tambahkan model.
	if err := c.BodyParser(&req); err != nil {
		return fail(c, fiber.StatusBadRequest, "body harus berupa JSON yang valid")
	}
...
func patchUser(c *fiber.Ctx) error {
	...
	var req model.PatchUserRequest // Tambahkan model.
	if err := c.BodyParser(&req); err != nil {
		return fail(c, fiber.StatusBadRequest, "body harus berupa JSON yang valid")
	}
